...
// Ticket related measures
var (
	TotalBytesPerTicket     = stats.Int64("openmatch.dev/total_bytes_per_ticket", "Total bytes per ticket", stats.UnitBytes)
	ExtensionBytesPerTicket = stats.Int64("openmatch.dev/extension_bytes_per_ticket", "Extension bytes per ticket", stats.UnitBytes)
...
var (
	...
	ExtensionBytesPerMatch = stats.Int64("openmatch.dev/extension_bytes_per_match", "Extension bytes per match", stats.UnitBytes)
	TicketsPerMatch        = stats.Int64("openmatch.dev/tickets_per_match", "Number of tickets per match", stats.UnitDimensionless)
)

// QueryService measures
var (
	...
	QueryCacheUpdatedItems   = stats.Int64("openmatch.dev/query/updated_cache_items", "Number of tickets added by query service in the last update", stats.UnitDimensionless)
	QueryCacheWaitingQueries = stats.Int64("openmatch.dev/query/waiting_requests", "Number of waiting requests between two cache updates", stats.UnitDimensionless)
...
func measureToView(s *stats.Measure, metricName, metricDesc string, aggregation *view.Aggregation, tags ...tag.Key) *view.View {
	v := &view.View{
		Name:        metricName,
	...
	err := view.Register(v)
	if err != nil {
		logger.WithError(err).Infof("cannot register view for metric: %s, it will not be reported", metricName)
	}
	return v
}
